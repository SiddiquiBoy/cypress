...
export class CanActivateViewCustomerGuard implements CanActivate {
  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      if (this.authenticationService.checkPermission(Permission.VIEW_CUSTOMER_DETAIL)) {
        ...
        const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.CUSTOMERS);
        if (nextRoute) {
