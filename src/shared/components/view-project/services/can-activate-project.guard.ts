...
export class CanActivateViewProjectGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.VIEW_PROJECT_DETAIL)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.PROJECTS);
      if (nextRoute) {
