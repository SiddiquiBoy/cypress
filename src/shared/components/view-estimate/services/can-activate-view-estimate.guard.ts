...
export class CanActivateViewEstimateGuard implements CanActivate {
  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      if (this.authenticationService.checkPermission(Permission.VIEW_ESTIMATE)) {
        ...
        const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.FOLLOW_UP_ESTIMATES);
        if (nextRoute) {
