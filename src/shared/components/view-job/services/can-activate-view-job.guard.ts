...
export class CanActivateViewJobGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.VIEW_JOB_DETAIL)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.JOBS);
      if (nextRoute) {
