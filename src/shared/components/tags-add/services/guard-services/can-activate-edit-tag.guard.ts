...
export class CanActivateEditTagGuard implements CanActivate {
  constructor(
  ...
  canActivate(
    ...
    | boolean
    | UrlTree {
    if (this.authenticationService.checkPermission(Permission.EDIT_TAG)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(
        RouterConstants.SETTINGS_OPERATIONS_TAGS
      );
      if (nextRoute) {
