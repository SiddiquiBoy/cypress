...
export class CanActivateEditCompanyGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.EDIT_COMPANY)) {
      ...
      if (!this.authenticationService.firstRedirect) {
        // this.notificationService.info('Permission Not Allowed', 'User does not have permission to visit the company add page');
      }
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.COMPANIES);
      if (nextRoute) {
