...
export class CanActivateEditTimesheetCodeGuard implements CanActivate {
  constructor(
  ...
  canActivate(
    ...
    | UrlTree {
    if (
      this.authenticationService.checkPermission(Permission.EDIT_TIMESHEET_CODE)
    ) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(
        RouterConstants.SETTINGS_OPERATIONS_TIMESHEETCODES
      );
      if (nextRoute) {
        this.router.navigate([nextRoute]);
      } else {
        this.notificationService.info(
          'No Routes To Navigate',
          'No Routes are defined for this user'
        );
      }
      return false;
    }
