...
export class CallCanActivateService implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean | UrlTree | Observable<boolean | UrlTree> | Promise<boolean | UrlTree> {
    // if (!this.authenticationService.checkRole('super-admin')) {
      ...
      if (!this.authenticationService.firstRedirect) {
        // this.notificationService.info('Permission Not Allowed', 'User does not have permission to visit the calls page');
      }
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.CALLS);
      if (nextRoute) {
