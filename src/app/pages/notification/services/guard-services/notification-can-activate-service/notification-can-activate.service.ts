...
export class NotificationCanActivateService implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean | UrlTree | Observable<boolean | UrlTree> | Promise<boolean | UrlTree> {
    // if (this.authenticationService.checkRole('super-admin') || this.authenticationService.checkRole('admin')) {
      if (this.authenticationService.checkPermission(Permission.MENU_NOTIFICATION)) {
      ...
      if (!this.authenticationService.firstRedirect) {
        // this.notificationService.info('Permission Not Allowed', 'User does not have permission to visit the notification page');
      }
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.NOTIFICATION);
      if (nextRoute) {
