...
export class InvoiceCanActivateService implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean | UrlTree | Observable<boolean | UrlTree> | Promise<boolean | UrlTree> {
    ...
    if (this.authenticationService.checkPermission(Permission.MENU_INVOICE)) {
      ...
      if (!this.authenticationService.firstRedirect) {
        // this.notificationService.info('Permission Not Allowed', 'User does not have permission to visit the invoice page');
      }
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.INVOICE);
      if (nextRoute) {
