...
export class CanActivateCompanyProfileGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.MENU_SETTINGS_COMPANY_PROFILE)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.SETTINGS_COMPANY_PROFILE);
      if (nextRoute) {
