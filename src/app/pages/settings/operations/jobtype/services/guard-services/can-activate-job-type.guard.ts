...
export class CanActivateJobTypeGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.MENU_OPERATIONS_JOB_TYPES)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.SETTINGS_OPERATIONS_JOBTYPE);
      if (nextRoute) {
