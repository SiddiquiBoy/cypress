...
export class CanActivateBusinessUnitGuard implements CanActivate {
  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      if (this.authenticationService.checkPermission(Permission.MENU_OPERATIONS_BUSINESS_UNITS)) {
        ...
        const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.SETTINGS_OPERATIONS_BUSINESSUNIT);
        if (nextRoute) {
