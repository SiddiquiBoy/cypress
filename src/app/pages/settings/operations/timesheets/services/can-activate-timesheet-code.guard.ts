...
export class CanActivateTimesheetCodeGuard implements CanActivate {
  ...
  canActivate(
    ...
    if (this.authenticationService.checkPermission(Permission.MENU_OPERATIONS_TIMESHEET_CODES)) {
      ...
      const nextRoute: string = this.mainUiService.getNextRoute(RouterConstants.SETTINGS_OPERATIONS_TIMESHEETCODES);
      if (nextRoute) {
